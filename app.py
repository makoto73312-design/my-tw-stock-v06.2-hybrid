...
def get_tickers_from_sheet(url):
    ...
    except Exception as e: return "2330, 2317, 2454, 2603, 3231", {}
...
def fetch_tw_fundamental_and_news(raw_id, processed_id, cloud_dict):
    TW_NAMES_DICT = {"2330":"台積電","2317":"鴻海","2454":"聯發科","2603":"長榮","3231":"緯創","2308":"台達電","2382":"廣達"}
    comp_name = cloud_dict.get(raw_id, TW_NAMES_DICT.get(raw_id, "台灣個股"))
    f_info = {"comp_name": comp_name, "pe": "-", "fcf": "-", "rev_growth": "-", "is_fcf_positive": True, "near_earnings": False, "news_alert": "🟢 無異常", "quality_tag": "一般"}
    try:
        tk = yf.Ticker(processed_id); info = tk.info or {}
        if comp_name == "台灣個股":
            short_name = info.get('shortName', raw_id)
            for kw in ["TAIWAN", "STOCK", "LTD", "INC", "CORPORATION", "COMPANY"]: short_name = short_name.upper().split(kw)[0].strip()
            f_info["comp_name"] = short_name
...
with tab_v061:
    if st.session_state.calculated:
        ...
        with st.expander(f"🚀 今日建議大膽建倉 ({len(buy_df)} 筆)", expanded=len(buy_df)>0): st.dataframe(buy_df[display_cols], use_container_width=True, hide_index=True) if not buy_df.empty else st.info("無觸發建倉")
        with st.expander(f"🔴 今日觸發防守賣出 ({len(sell_df)} 筆)", expanded=len(sell_df)>0): st.dataframe(sell_df[display_cols], use_container_width=True, hide_index=True) if not sell_df.empty else st.info("無觸發賣出")
        with st.expander(f"📦 獲利續抱中 ({len(hold_df)} 筆)"): st.dataframe(hold_df[display_cols], use_container_width=True, hide_index=True) if not hold_df.empty else st.info("無持有")
        with st.expander(f"🛡️ 風控安全閥攔截 ({len(risk_df)} 筆)", expanded=len(risk_df)>0): st.dataframe(risk_df[display_cols], use_container_width=True, hide_index=True) if not risk_df.empty else st.info("無攔截")
        with st.expander(f"💵 空手觀望 ({len(cash_df)} 筆)"): st.dataframe(cash_df[display_cols], use_container_width=True, hide_index=True) if not cash_df.empty else pass

        st.divider(); st.subheader("📋 完整多因子對照總表")
        def apply_block_shading(df):
            styles = pd.DataFrame('', index=df.index, columns=df.columns)
            for i, ticker in enumerate(df["台股代號"].unique()): styles.loc[df["台股代號"] == ticker, :] = 'background-color: rgba(128, 128, 128, 0.16)' if i % 2 == 0 else 'background-color: rgba(0, 0, 0, 0)'
            return styles
...
with tab_matrix:
    ...
    with st.expander("📖 點擊展開：【七維戰術矩陣】手冊", expanded=False): st.markdown("（手冊說明與原本相同）")
    if st.session_state.calculated:
        df_res, matrix_data = st.session_state.final_df, []
        for t in df_res['台股代號'].unique():
            ...
            matrix_data.append({
                ...
                "❌洗盤怪獸": len([s for s in sub.index if 'BUY' in sub.loc[s, '倉位狀態']]) >= 2 and all(sub.loc[s, '推薦指數'] == '❌ 不推薦' for s in sub.index if 'BUY' in sub.loc[s, '倉位狀態'])
            })
        m_df = pd.DataFrame(matrix_data)
        col_m1, col_m2, col_m3, col_m4, col_m5, col_m6, col_m7 = st.columns(7)
        for i, (k, col) in enumerate(zip(['🔥全面共振','🌊波段突破','🕵️籌碼吸籌','🛒價值窪地','⚠️頭部背離','🔴集體撤退','❌洗盤怪獸'], [col_m1, col_m2, col_m3, col_m4, col_m5, col_m6, col_m7])):
            col.metric(k, f"{len(m_df[m_df[k]])} 檔")
        st.divider()
        for k, title in zip(['🔥全面共振','🌊波段突破','🕵️籌碼吸籌','🛒價值窪地','⚠️頭部背離','🔴集體撤退','❌洗盤怪獸'], ["1. 全面共振多頭", "2. 高勝率波段突破", "3. 籌碼大戶潛伏", "4. 價值超跌窪地", "5. 動能/籌碼頭部背離", "6. 多頭集體撤退", "7. 高波動洗盤怪獸"]):
            sub_df = m_df[m_df[k]][['台股代號', '公司名稱', '當前股價']]
            with st.expander(f"{title} — {len(sub_df)} 檔", expanded=len(sub_df)>0): st.dataframe(sub_df, use_container_width=True, hide_index=True) if not sub_df.empty else st.info("無訊號")
