...
st.markdown(
    "已實裝 **V06.2 五維獨立介面**：**動作分類看板**、**七維戰術矩陣**、**歷史驗證線圖**、**實盤前向驗證 (昨日訊號vs今日實況)** 與 **多線程平行加速引擎**"
)
...
def get_tickers_from_sheet(url):
  try:
    ...
    valid_tickers = [
        t
        for t in tickers
        if len(t) > 0 and t != "股票代號" and not t.startswith("202")
    ]
    return (
        ", ".join(valid_tickers)
        if valid_tickers
        else "2330, 2317, 2454, 2603, 3231"
    ), custom_names_dict
  except Exception:
...
tickers_input = st.sidebar.text_area(
    "📡 當前雲端同步清單 (可貼 Excel)", default_tickers, height=120
)
...
def fetch_tw_macro_environment():
  try:
    vix_df, tw_df = yf.download(
        "^VIX", period="5d", progress=False
    ), yf.download("^TWII", period="1y", progress=False)
    vix_clean, tw_clean = vix_df.dropna(subset=["Close"]), tw_df.dropna(
        subset=["Close"]
    )
    vix_val = float(vix_clean["Close"].iloc[-1]) if not vix_clean.empty else 18.0
    tw_bull = (
        float(tw_clean["Close"].iloc[-1])
        >= float(tw_clean["Close"].rolling(200).mean().iloc[-1])
        if not tw_clean.empty
        else True
    )
    if vix_val >= 25 or not tw_bull:
      return vix_val, tw_bull, "🥶 極度謹慎型 (大盤空頭/高恐慌)"
    elif vix_val <= 15 and tw_bull:
      return vix_val, tw_bull, "🚀 大膽進攻型 (晴天多頭行情)"
...
def fetch_tw_fundamental_and_news(raw_id, processed_id, cloud_dict):
  ...
  try:
    ...
    if fcf and fcf > 0 and rev_g and rev_g > 0.10:
    ...
    bad_count = sum(
        ...
        if any(
            ...
            for kw in ["LAWSUIT", "PROBE", "DOWNGRADE", "MISSED"]
        )
    )
    if bad_count >= 1:
...
def run_backtest_engine(df, strategy_name, days, posture, fund_info):
  valid_df = (
      df.dropna(subset=["200MA", "ROC14", "MACD_Hist", "RSI_14", "Vol_MA20"])
      .tail(days)
      .copy()
  )
  if len(valid_df) < 5:
  ...
  if total_return > 0 and total_trades > 0:
    if total_return >= 0.20 and final_win_rate >= 0.52:
      stars = "⭐⭐⭐⭐⭐"
    elif total_return >= 0.12 or final_win_rate >= 0.48:
      stars = "⭐⭐⭐⭐"
    else:
      stars = "⭐⭐"
  ...
  if has_position:
    ...
    sl_price_str = (
        f"NT${highest_price_since_entry * (1 - stop_loss_pct):.2f}"
        if "D:" not in strategy_name
        else f"NT${max(entry_price * 0.95, m200s[-1]):.2f}"
    )
  else:
    current_status = (
        "🔴 今日觸發防守賣出 (SELL)"
...
# ⚡ 多線程 Worker 函數
def process_single_stock_tw(
    raw_name, ticker, cloud_dict, backtest_days, posture, strategies
):
  try:
...
col_v2.metric(
    "台灣加權指數 (大盤)",
    "年線之上 (多頭)" if is_tw_bull else "跌破年線 (空頭)",
)
col_v3.metric("系統自動環境姿態", market_posture)
st.divider()

if st.button(
    "🚀 啟動 V06.2 台股全自動多因子掃描引擎 (⚡ 多線程平行加速)",
  ...
  with st.spinner(
    ...
    st.success(
        "📊 V06.2 台股多因子矩陣與前向驗證計算完成！請至下方各分頁切換檢視。"
    )
...
with tab_v061:
  if st.session_state.calculated:
    ...
    st.info(
        "💡 請按下上方「🚀 啟動 V06.2 台股全自動多因子掃描引擎」按鈕開始運算。"
    )
  ...
  if st.session_state.calculated:
    ...
    for t in df_res["台股代號"].unique():
      ...
      matrix_data.append({
          ...
          "公司名稱": (
              sub["公司名稱"].iloc[0]
              if "公司名稱" in sub.columns
              else "台灣個股"
          ),
          "當前股價": (
              sub["當前股價"].iloc[0] if "當前股價" in sub.columns else "-"
          ),
          "🔥全面共振": (
              ("BUY" in st_A)
